# Rocco 的书签

[我的博客](https://github.com/zh-rocco/fe-notes/issues)

## 基础

- [What-happens-when 的中文翻译](https://github.com/skyline75489/what-happens-when-zh_CN)

## TypeScript

- [Let's develop Vue app with TypeScript](https://github.com/zh-rocco/vue-typescript-handbook)
- [TypeScript 入门教程](https://github.com/xcatliu/typescript-tutorial)
- [TypeScript 使用手册](https://github.com/zhongsp/TypeScript)
- [TypeScript Deep Dive 中文版](https://github.com/jkchao/typescript-book-chinese)
- [A collection of awesome TypeScript resources for client-side and server-side development](https://github.com/semlinker/awesome-typescript)
- [The definitive guide to TypeScript and possibly the best TypeScript book ](https://github.com/basarat/typescript-book)
- [Design pattern implementations in TypeScript](https://github.com/torokmark/design_patterns_in_typescript)
- [Clean Code concepts adapted for TypeScript](https://github.com/labs42io/clean-code-typescript)
- [Style guide for adding type definitions to my npm packages](https://github.com/sindresorhus/typescript-definition-style-guide)
- [TypeScript + 大型项目实战](https://juejin.im/post/5b54886ce51d45198f5c75d7)
- [如使用Typescript撸Vue（Vue2 + TS +TSX+CSS module）](https://zhuanlan.zhihu.com/p/58351868)
- [TypeScript 在 Vue 的实践](https://zhuanlan.zhihu.com/p/50179498)
- [TypeScript 强大的类型别名](https://juejin.im/post/5c2f87ce5188252593122c98)
- [巧用 Typescript](https://zhuanlan.zhihu.com/p/39620591)

## 函数式编程

- [函数式编程术语及示例](https://github.com/shfshanyue/fp-jargon-zh)
- [Pragmatic, balanced FP in JavaScript](https://github.com/getify/Functional-Light-JS)
- [ramda - Practical functional Javascript](https://github.com/ramda/ramda)

## Vue

- [Vue 技术内幕](http://hcysun.me/vue-design/)
- [Vue.js 技术揭秘](https://ustbhuangyi.github.io/vue-analysis/)
- [vue 源码逐行注释分析+40 多 m 的 vue 源码程序流程图思维导图](https://github.com/qq281113270/vue-ddf-)
- [看Vue源码，不会调试不行啊](https://zhuanlan.zhihu.com/p/53206857)

## CSS

- [30-seconds-of-css - A curated collection of useful CSS snippets](https://github.com/atomiks/30-seconds-of-css)
- [iCSS - 谈谈一些有趣的 CSS 话题](https://github.com/chokcoco/iCSS)
- [magicCss - CSS3 奇思妙想，单标签实现各类图形](https://github.com/chokcoco/magicCss)
- [css-protips - A collection of tips to help take your CSS skills pro](https://github.com/AllThingsSmitty/css-protips)
- [Devices.css - Modern devices in pure CSS](https://picturepan2.github.io/devices.css/)

## 性能与安全

- [网站性能优化实战——从 12.67s 到 1.06s 的故事](http://imweb.io/topic/5b6fd3c13cb5a02f33c013bd)
- [前端性能与异常上报](https://zhuanlan.zhihu.com/p/40830370)

## 工具库

- [flexsearch - Next-Generation full text search library for Browser and Node.js](https://github.com/nextapps-de/flexsearch)

## 开发工具

- [whistle - HTTP, HTTPS, WebSocket debugging proxy](https://github.com/avwo/whistle)
- [spy-debugger - 微信调试，各种 WebView 样式调试、手机浏览器的页面真机调试](https://github.com/wuchangming/spy-debugger)

## Git

- [高质量的 Git 中文教程](https://github.com/geeeeeeeeek/git-recipes)
- [gitmoji-cli - A gitmoji interactive command line tool for using emojis on commits](https://github.com/carloscuesta/gitmoji-cli)
- [github-webhook-handler - Node.js web handler / middleware for processing GitHub Webhooks](https://github.com/rvagg/github-webhook-handler)

## 单元测试

- [一步一步介绍如何给项目添加单元测试](https://github.com/tank0317/unit-test-demo)

## 打包工具

- [webpack 优秀中文文章(印记中文)](https://github.com/webpack-china/awesome-webpack-cn)
- [全面的 Webpack 教程《深入浅出 Webpack》电子书](https://github.com/gwuhaolin/dive-into-webpack)
- [手摸手，带你用合理的姿势使用 webpack4（下）](https://segmentfault.com/a/1190000015919928)
- [理解webpack原理，手写一个100行的webpack](https://zhuanlan.zhihu.com/p/58151131)

## NodeJS

- [Delightful Node.js packages and resources](https://github.com/sindresorhus/awesome-nodejs)
- [The largest Node.js best practices list](https://github.com/i0natan/nodebestpractices)
- [深入理解 Node.js：核心思想与源码分析](https://github.com/yjhjstz/deep-into-node)
- [Nodejs 学习笔记以及经验总结，公众号"程序猿小卡"](https://github.com/chyingp/nodejs-learning-guide)
- [How to pass the Node.js interview of ElemeFE](https://github.com/ElemeFE/node-interview)
- [《一起学 Node.js》](https://github.com/nswbmw/N-blog)

### 框架

- [egg - Born to build better enterprise frameworks and apps with Node.js & Koa](https://github.com/eggjs/egg)
- [NestJS - A progressive Node.js framework for building efficient and scalable server-side applications, heavily inspired by Angular.](https://github.com/nestjs/nest)
- [notadd - 基于 Nest.js 的微服务开发架构](https://github.com/notadd/notadd)
- [typeorm - ORM for TypeScript and JavaScript (ES7, ES6, ES5)](https://github.com/typeorm/typeorm)
- [strapi - Open source Node.js Headless CMS to easily build customisable APIs](https://github.com/strapi/strapi)

### 工具/插件

- [execa -A better `child_process`](https://github.com/sindresorhus/execa)
- [prompts - Lightweight, beautiful and user-friendly interactive prompts](https://github.com/terkelg/prompts)
- [draftlog - Create updatable log lines into the terminal, and give life to your logs](https://github.com/ivanseidel/node-draftlog)
- [signale - Hackable console logger](https://github.com/klaussinani/signale)
- [ndb - ndb is an improved debugging experience for Node.js, enabled by Chrome DevTools](https://github.com/GoogleChromeLabs/ndb)
- [shelljs - Portable Unix shell commands for Node.js](https://github.com/shelljs/shelljs)

## 桌面端

- [electron](https://github.com/electron/electron)
- [nativefier - Make any web page a desktop application](https://github.com/jiahaog/nativefier)
